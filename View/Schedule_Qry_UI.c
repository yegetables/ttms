...
void Schedule_UI_ListAll(void)
{
    ...
    do
    {
        ...
        printf("%10s  %10s  %10s  %10s  %30s  %15s  %15s\n", "Play_id", "id",
               "studio_id", "seat_count", "Schedule_time", "Play_name",
               "Studio_name");
        printf(
            "------------------------------------------------------------------"
            "\n");
        Paging_ViewPage_ForEach(head, paging, schedule_node_t, pos, i)
        {
            ...
            printf(
                "%10d  %10d  %10d  %10d  %d--%d--%d--%d--%d--%d  %15s  %15s\n",
                pos->data.play_id, pos->data.id, pos->data.studio_id,
                pos->data.seat_count, pos->data.date.year, pos->data.date.month,
                pos->data.date.day, pos->data.time.hour, pos->data.time.minute,
                pos->data.time.second, play.name, stu.name);
        }
        printf(
    ...
    while (list_p != NULL)
    {
        ...
        while (list_s != NULL)
        {
            ...
            printf("%10s  %10s  %10s  %10s  %30s\n", "Play_id", "id",
                   "studio_id", "seat_count", "Schedule time");
            printf(
                "--------------------------------------------------------------"
                "----"
                "\n");
            printf("%10d  %10d  %10d  %10d  %d--%d--%d--%d--%d--%d\n",
                   list_s->data.play_id, list_s->data.id,
                   list_s->data.studio_id, list_s->data.seat_count,
                   list_s->data.date.year, list_s->data.date.month,
                   list_s->data.date.day, list_s->data.time.hour,
                   list_s->data.time.minute, list_s->data.time.second);
